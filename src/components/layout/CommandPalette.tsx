...
export function CommandPalette() {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 border-b border-hairline px-4 py-3.5 bg-surface-container-low/40">
        <Search className="h-5 w-5 text-charcoal-subtle" />
        <input
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search screens, actions, or switch personas..."
          className="flex-1 bg-transparent text-sm text-charcoal placeholder:text-charcoal-subtle focus:outline-none"
          autoFocus
        />
        <kbd className="rounded border border-hairline bg-white px-2 py-0.5 font-mono text-[10px] text-charcoal-subtle">
          ESC
        </kbd>
      </div>
      ...
      <div className="max-h-80 overflow-y-auto p-2 space-y-3">
        {filteredNav.length > 0 && (
          <div>
            <p className="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-charcoal-subtle">
            ...
            <div className="space-y-0.5">
              {filteredNav.map((item) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-xs font-medium text-charcoal hover:bg-saffron-50 hover:text-primary-container transition-colors"
                >
                  <div className="flex items-center gap-2.5">
                    <span className="text-charcoal-subtle">{item.icon}</span>
                    <span>{item.label}</span>
                  </div>
                  <ArrowRight className="h-3.5 w-3.5 text-charcoal-subtle opacity-50" />
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-0.5">
              {filteredActions.map((item) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-xs font-medium text-charcoal hover:bg-surface-container transition-colors"
                >
                  <span>{item.label}</span>
                  <span className="rounded-md bg-saffron-100 px-1.5 py-0.5 text-[10px] font-semibold text-saffron-800">
                    {item.badge}
                  </span>
                </button>
              ))}
        ...
        {filteredNav.length === 0 && filteredActions.length === 0 && (
          <div className="py-8 text-center text-xs text-charcoal-subtle">
